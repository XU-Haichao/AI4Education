...
function createEmptyMedia() {
    return { links: [], images: [], videos: [], documents: [] };
}
...
function sanitizePageId(pageId) {
    const safe = String(pageId || '').trim().toLowerCase().replace(/[^a-z0-9-_]/g, '');
    return safe || null;
}
...
async function writeStore(data) {
    ...
    await fs.promises.writeFile(DATA_FILE, JSON.stringify(safe, null, 2), 'utf8');
}

function getOrCreatePage(store, pageId) {
    if (!store.pages[pageId]) store.pages[pageId] = { content: '', media: createEmptyMedia() };
    if (typeof store.pages[pageId].content !== 'string') store.pages[pageId].content = '';
...
async function handleApiRequest(req, res, pathname) {
    if (req.method === 'GET' && pathname === '/api/health') {
        sendJson(res, 200, { ok: true, storage: 'admin_uploads', dataFile: 'admin_uploads/content-data.json' });
        return true;
    }
    ...
    if (pageRoute && req.method === 'GET') {
        ...
        if (!pageId) return sendJson(res, 400, { error: 'Invalid pageId.' }), true;
        const store = await readStore();
        const exists = Boolean(store.pages[pageId]);
        const page = exists ? getOrCreatePage(store, pageId) : { content: '', media: createEmptyMedia() };
        sendJson(res, 200, { pageId, exists, content: page.content, media: page.media });
        return true;
    }
    ...
    if (mediaRoute && req.method === 'PUT') {
        ...
        const store = await readStore();
        const page = getOrCreatePage(store, pageId);
        page.media = normalizeMedia(body.media);
        await writeStore(store);
        sendJson(res, 200, { ok: true });
        return true;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/upload') {
        ...
        if (!pageId || !categoryInfo || !parsed) return sendJson(res, 400, { error: 'Invalid upload payload.' }), true;

        const safeName = sanitizeBaseName(body.originalName || body.displayName || 'file');
        const extOriginal = path.extname(safeName).toLowerCase();
        const ext = extOriginal || getExtensionFromMime(parsed.mimeType) || '.bin';
        const base = sanitizeBaseName(path.basename(safeName, extOriginal || path.extname(safeName)));
        const unique = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${base}${ext}`;

        const folder = path.join(FILES_ROOT, pageId, categoryInfo.folder);
        await fs.promises.mkdir(folder, { recursive: true });
        const target = path.join(folder, unique);
        await fs.promises.writeFile(target, parsed.buffer);

        const relativeUrl = `/${path.relative(ROOT_DIR, target).split(path.sep).join('/')}`;
        const docType = categoryInfo.kind === 'documents' ? inferDocType(unique, body.docType || 'pdf') : '';
        sendJson(res, 200, { ok: true, url: relativeUrl, type: docType || undefined });
        return true;
    }
...
async function serveStaticFile(req, res, pathname) {
    ...
    if (!fs.existsSync(finalPath)) return sendText(res, 404, 'Not found.');
...
async function start() {
    ...
    server.listen(PORT, () => {
        console.log(`Quantum local server running at http://localhost:${PORT}`);
